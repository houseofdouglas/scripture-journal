...
3a. **Geometry corrections (per page).** Textract's reading order is trusted for the main column flow, but two corrections use block bounding boxes so a callout box embedded mid-column doesn't corrupt the text:
   - **Figure-label drop**: a kept text block that horizontally overlaps a `LAYOUT_FIGURE`/`LAYOUT_TABLE` (by > 50% of the narrower box) and sits within that figure's vertical band is a figure/table label (axis labels, "Source: …", "Delivery on criteria") — dropped, not emitted.
   - **Sidebar relocation**: a block wider than 1.4× the document's median content-block width (the single-column grid width, estimated once across all pages so diagram-heavy pages don't skew it) is a multi-column callout/sidebar. Such blocks — and any heading sitting directly on top of one — are moved to the end of their page. Removing an embedded sidebar lets the two halves of a body sentence it was sitting between fall adjacent and re-merge (4b). A merge barrier prevents main text from folding into the relocated sidebar. Found post-launch on the reference document's Wal-Mart sidebar (p6), where the callout split "…Nor will | [sidebar] | the company offer penny stocks…".
...
4. Post-processing on the kept blocks:
   ...
   - (b) **Continuation merge**: a block that ends mid-sentence, whose successor starts with a lowercase letter, is joined with the successor into one paragraph (repairs mid-sentence splits at column/page boundaries; a trailing line-break hyphen joins without a space). "Ends mid-sentence" requires one of two signals — the final word starts lowercase (optionally ending `,`/`;`/`-`), or the block is ≥ 150 characters and lacks terminal punctuation (`.`, `!`, `?`, `:`, `"`, `”`). Lacking terminal punctuation alone is deliberately insufficient: short caption/label blocks ("Source: …", figure text, mastheads) also end unpunctuated, and merging one buries real body text inside a caption (bug found post-launch on the reference document's Wal-Mart sidebar).
